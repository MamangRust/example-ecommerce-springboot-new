...
public class OrderQueryImplService implements OrderQueryService {
        ...
        public ApiResponsePagination<List<OrderResponse>> findByMerchantId(FindAllOrderByMerchantRequest req) {
                ...
                try {
                        Pageable pageable = PageRequest.of(page, pageSize);
                        Page<Order> orderPage = orderQueryRepository.findOrdersByMerchant(keyword,
        ...
        @Override
        public ApiResponse<OrderRelationResponse> findOrderRelation(Integer id) {
                log.info("🔍 Finding order relation by id={}", id);

                try {
                        OrderRelation orderRelation = orderQueryRepository.findOrderRelations(id.longValue());

                        if (orderRelation == null) {
                                log.warn("❌ Order relation not found with id={}", id);
                                return ApiResponse.<OrderRelationResponse>builder()
                                                .status("error")
                                                .message("Order relation not found")
                                                .data(null)
                                                .build();
                        }

                        OrderRelationResponse response = OrderRelationResponse.from(orderRelation);

                        return ApiResponse.<OrderRelationResponse>builder()
                                        .status("success")
                                        .message("Order relation retrieved successfully")
                                        .data(response)
                                        .build();

                } catch (Exception e) {
                        log.error("💥 Failed to fetch order relation by id={}", id, e);
                        return ApiResponse.<OrderRelationResponse>builder()
                                        .status("error")
                                        .message("Failed to fetch order relation")
                                        .data(null)
                                        .build();
                }
        }
